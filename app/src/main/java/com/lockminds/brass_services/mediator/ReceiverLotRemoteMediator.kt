...
class ReceiverLotRemoteMediator(
    private val context: Context,
    private val query: String,
    private val service: ReceiverLotService,
    private val appDatabase: AppDatabase
) : RemoteMediator<Int, ReceiverLot>() {

    @SuppressLint("HardwareIds")
    override suspend fun load(loadType: LoadType, state: PagingState<Int, ReceiverLot>): MediatorResult {
        ...
        try {
            ...
            appDatabase.withTransaction {
                ...
                if (loadType == LoadType.REFRESH) {
                    appDatabase.receiverLotDao().clearRemoteKeys()
                    appDatabase.receiverLotDao().emptyTable()
                }
                val prevKey = if (page == GITHUB_STARTING_PAGE_INDEX) null else page - 1
                val nextKey = if (endOfPaginationReached) null else page + 1
                val keys = repos.map {
                    ReceiverLotRemoteKeys(id = it.id, prevKey = prevKey, nextKey = nextKey)
                }
                appDatabase.receiverLotDao().insertKeys(keys)
                appDatabase.receiverLotDao().insertItems(repos)
            }
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, ReceiverLot>): ReceiverLotRemoteKeys? {
        // Get the last page that was retrieved, that contained items.
        // From that last page, get the last item
        return state.pages.lastOrNull() { it.data.isNotEmpty() }?.data?.lastOrNull()
            ?.let { repo ->
                // Get the remote keys of the last item retrieved
                appDatabase.receiverLotDao().getRemoteKey(repo.id.toString())
            }
    }

    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, ReceiverLot>): ReceiverLotRemoteKeys? {
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(
        state: PagingState<Int, ReceiverLot>
    ): ReceiverLotRemoteKeys? {
        // The paging library is trying to load data after the anchor position
        // Get the item closest to the anchor position
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.id?.let { repoId ->
                appDatabase.receiverLotDao().getRemoteKey(repoId.toString())
            }
        }
    }
